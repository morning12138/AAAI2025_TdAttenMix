...
from timm.models.layers import PatchEmbed, Mlp, DropPath, trunc_normal_#,PatchEmbed
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, task_param=1, return_all_features=False, y=None):
        input = x
        output_each_iter = []
        ...
        if y is not None:
            cos_sim = F.normalize(x, dim=-1) @ F.normalize(self.head.weight[y][..., None], dim=1)
        else:
            cos_sim = F.normalize(x, dim=-1) @ F.normalize(self.prompt[None, ..., None], dim=1)  # B, N, 1
        mask = cos_sim.clamp(0, 1)
        x = x * mask * self.feedback_aug
        td = self.feedback(x, task_param)
        attn, in_var, out_var = self.forward_features(input, td)
        x = self.forward_head(attn)
        output_each_iter.append(x)

        if y is not None:
            cos_sim = F.normalize(attn, dim=-1) @ F.normalize(self.head.weight[y][..., None], dim=1)
        else:
            cos_sim = F.normalize(attn, dim=-1) @ F.normalize(self.prompt[None, ..., None], dim=1)  # B, N, 1
        mask = cos_sim.clamp(0, 1)
        attn = mask[:, self.num_prefix_tokens:].view(mask.shape[0], int(196 ** 0.5), int(196 ** 0.5))
